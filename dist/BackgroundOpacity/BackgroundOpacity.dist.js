...
const BackgroundOpacity = ({ name, attributes, setAttributes }) => {
    ...
    const defaultValue = comet?.defaults?.[name.replace('comet/', '')]?.backgroundOpacity
        ...
        ?? 0;
    return (React.createElement(RangeControl, { __next40pxDefaultSize: true, initialPosition: attributes.backgroundOpacity ?? defaultValue, onChange: (value) => setAttributes({ backgroundOpacity: value }), label: "Background opacity", max: 100, min: 50, allowReset: true, resetFallbackValue: defaultValue }));
};
